...
if __name__ == '__main__':
    ...
    model.add(layers.Conv2D(128, (3, 3), activation='relu', input_shape=image_size + (3,)))
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(filepath=os.path.join(path_to_save_model, f'{model_name}.hdf5'),
    ...
    model = keras.models.load_model(os.path.join(path_to_save_model, f'{model_name}.hdf5'))
    ...
    cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_matrix, display_labels=['no transport',
                                                                                                   'transport'])
    cm_display.plot()
