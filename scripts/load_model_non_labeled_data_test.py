    ...
    for model_name in models_names:
        ...
        for class_label in os.listdir(test_data_path):
            ...
            for image_name in tqdm(
                os.listdir(os.path.join(test_data_path, class_label))
            ):
                ...
                if prediction > 0.5 and class_label == "0":
                    ...
                    fullpath = (
                        f"models_bad_classification\\{model_name}\\no_car\\{image_name}"
                    )
                    cv2.imwrite(fullpath, image)
                else:
                    fullpath = (
                        f"models_bad_classification\\{model_name}\\car\\{image_name}"
                    )
                    cv2.imwrite(fullpath, image)

    df = pd.DataFrame.from_dict(rows)
    df_pivot = df.pivot_table(
        index="full_image_path", values="score", columns="model_name"
    ).reset_index()
    return df_pivot
