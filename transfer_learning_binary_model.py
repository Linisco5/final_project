...
if __name__ == '__main__':
    ...
    x = layers.Conv2D(32, (3, 3), activation='relu', input_shape=image_size + (3,))(base_model.output)
    ...
    cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_matrix, display_labels=['no transport',
                                                                                                   'transport'])
    cm_display.plot()
    plt.savefig(os.path.join(path_to_save_plots, f'confussion_mtx_{model_name_to_save}.png'))
    plt.show()
